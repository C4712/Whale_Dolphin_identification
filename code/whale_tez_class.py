...
class HappyWhaleModel(tez.Model):    
    def __init__(self):
        super().__init__()
        self.effnet = model_whale(num_classes=num_classes, inchannels=3, model_name='senet154').cuda()
...
if __name__ == "__main__":
    current_fold = 0
    dfx = pd.read_csv(os.path.join(INPUT_PATH, "train_fold.csv"))
    ...
    es = EarlyStopping(
        monitor="valid_loss",
        model_path=os.path.join(MODEL_PATH, MODEL_NAME + f"_fold__{current_fold}.pth"),
        patience=3,
        mode="min",
    )
    model.fit(
    ...
    model.save(os.path.join(MODEL_PATH, MODEL_NAME + f"_fold_{current_fold}.pth"))
